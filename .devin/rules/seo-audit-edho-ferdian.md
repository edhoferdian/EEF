...
7. **Core Web Vitals as an SEO signal** — check LCP/INP/CLS **as a ranking
   ...
   duplicating the work — on Claude Code, delegate to that skill's own
   agent form directly (this skill's agent carries `Agent` in its
   `tools:` for exactly this hand-off); on any other harness, or when no
   delegation primitive exists, invoke the skill directly instead.

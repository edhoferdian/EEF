...
description: >-
  Behavior-preserving refactoring workflow that actively rewrites code for
  clarity — extracting overlong functions, flattening deep nesting,
  consolidating duplicated logic, AND removing over-engineered "just in
  case" abstractions — always gated on a passing test suite (or a
  characterization test written first) so behavior never changes. Use
  whenever the user wants code actually SIMPLIFIED or REFACTORED, not just
  reviewed: "sederhanakan kode ini", "refactor biar lebih rapi", "reduce
  complexity", "ini over-engineered". A read-only finding about the same
  issues (CQ-01/CQ-04/CQ-04b in `review-checklist.md`) is
  `code-review-edho-ferdian`'s job, not this skill's — see the scope table
  below for the exact boundary with that skill, `dead-code-cleanup-edho-
  ferdian`, and `build-fix-edho-ferdian`.
---
...
## Surgical changes (carve-out — see skill-authoring-edho-ferdian's canonical contract)

This skill is a deliberate, stated exception to the default in
`skill-authoring-edho-ferdian` §10 ("touch only what the user's current
instruction requires"), not an adopter of it: this skill's entire purpose is
non-surgical rewriting — restructuring already-working, adjacent code for
clarity is the job, not a side effect to avoid. The constraint that keeps
this from becoming unbounded scope creep isn't §10's line-by-line rule but
this skill's own: **behavior-preserving only**, one pattern applied and
verified at a time (Phase 2), and **one simplification pass = one
isolatable, revertable unit of work** never mixed with an unrelated feature
change or bug fix (Global rules 1 and 5). Full contract:

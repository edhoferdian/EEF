...
two separate agents, delegated by whatever is orchestrating this review
(dev-kickoff-edho-ferdian, another agent, or the user), each getting only
what its role needs: A gets the code and produces the draft; B gets the
code and A's draft report, never A's internal reasoning. This is a real
independence guarantee, not a role-play framing. On a harness with no
delegation primitive, role-play the two parts sequentially in one
context — less independent, still valuable. Note which mode you used in

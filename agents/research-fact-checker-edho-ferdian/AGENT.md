...
description: >-
  An independent citation audit of research-ops-edho-ferdian's synthesized
  report, split out as its own delegate specifically so it never inherits
  the synthesizer's confidence about its own claims — the skill's own
  stated failure mode is "a confident paragraph where the reader cannot
  tell which sentence came from a source," which a self-check can't fully
  catch precisely because the self-checker already believes its own
  report. Delegate here after Phase 4 (Report) produces a draft, before
  Phase 5 (Reflection) is treated as complete. On a harness without
